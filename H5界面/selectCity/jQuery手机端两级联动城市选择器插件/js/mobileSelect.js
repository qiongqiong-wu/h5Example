...
window.MobileSelect = ( function () {
	...
	MobileSelect.prototype = {
		...
		init: function ( config ) {
			...
			_this.transitionEnd = config.transitionEnd ? config.transitionEnd :
				function () {};
			...
			_this.ensureBtn.addEventListener( 'click', function () {
				...
				for ( var i = 0; i < _this.wheel.length; i++ ) {
					i == _this.wheel.length - 1 ? tempValue += _this.getValue( i ) :
			...
			for ( var i = 0; i < wheelsData.length; i++ ) { //列
				...
				if ( _this.jsonType ) {
					for ( var j = 0; j < wheelsData[ i ].data.length; j++ ) { //行
						tempHTML += '<li data-id="' + wheelsData[ i ].data[ j ].id + '">' +
							wheelsData[ i ].data[ j ].value + '</li>';
					}
				} else {
					for ( var j = 0; j < wheelsData[ i ].data.length; j++ ) { //行
				...
				( function ( i ) {
					...
					if ( _this.wheel[ i ] ) {
						...
						for ( var j = 0; j < _this.displayJson[ i ].length; j++ ) { //行
							tempHTML += '<li data-id="' + _this.displayJson[ i ][ j ].id + '">' +
								_this.displayJson[ i ][ j ].value + '</li>';
						}
		...
		getJson: function () {
			...
			if ( _this.cascade ) {
				...
				for ( var i = 0; i < _this.curDistance.length; i++ ) {
					temp.push( _this.wheelsData[ i ].data[ _this.getIndex( _this.curDistance[
						i ] ) ] );
				}
			} else {
				for ( var i = 0; i < _this.curDistance.length; i++ ) {
		...
		updateCurDistance: function ( theSlider, index ) {
			this.curDistance[ index ] = parseInt( theSlider.style.transform.split(
				',' )[ 1 ] );
		},
		...
		touch: function ( event, theSlider, index ) {
			...
			switch ( event.type ) {
				...
				case "touchend":
					...
					_this.curDistance[ index ] = _this.fixPosition( _this.curDistance[
						index ] );
					_this.movePosition( theSlider, _this.curDistance[ index ] );
					_this.oversizeBorder = -( theSlider.getElementsByTagName( 'li' ).length -
						3 ) * _this.liHeight;
