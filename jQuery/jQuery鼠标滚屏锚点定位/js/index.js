...
jQuery(document).ready(function($) {
	...
	$(document).mousewheel(function(event,delta){
		if (canscroll) {
			...
			canscroll = false;
	 		...
	 		//添加菜单当前色
	 		$(".head ul li").eq($key).addClass('current').siblings().removeClass('current');
	...
	$(".head ul li").click(function(event) {
		$key = $(this).index();
		$(".head ul li").eq($(this).index()).addClass('current').siblings().removeClass('current');
		 $("body,html").stop().animate({
...
function change_size(){
	...
	var size = Number($(".head ul li").size());
